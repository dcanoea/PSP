...
public class JFrameVentana extends javax.swing.JFrame {
    ...
    private void accionBoton(JButton boton) {
        if (boton.getText().equals("Sin hilos auxiliares")) {
            for (int i = 1; i <= 20; i++) {
                marcador.valor = i;
                marcador.repaint();
            }
        } else if (boton.getText().equals("Con hilo auxiliar")) {
            new HiloAuxiliar(false, marcador).start();
        } else if (boton.getText().equals("Con hilo y espera")) {
            new HiloAuxiliar(true, marcador).start();
        } else if (boton.getText().equals("Borrar marcador")) {
            marcador.valor = 0;
            marcador.repaint();
        }
    }
